...
public class ProductDetail extends JPanel{
	...
	JButton bt_update,bt_list,bt_delete;
	Product product;
	ProductVO vo;
	...
	public ProductDetail(Product product) {
		...
		p_content.setPreferredSize(new Dimension(AdminMain.WIDTH-200, AdminMain.HEIGHT-400));
		for(int i=0;i<title.length;i++) {
		...
		this.add(p_content);//현재 패널에 폼컨테이너 부착
		...
		bt_delete.addActionListener((e)->{
			if(deleteProduct() != 0) {
				JOptionPane.showMessageDialog(ProductDetail.this, "삭제하였습니다");
	...
	public int deleteProduct() {
		PreparedStatement pstmt = null;
		int result = 0 ;
		String sql = "delete from product where product_id = " + vo.getProduct_id();
