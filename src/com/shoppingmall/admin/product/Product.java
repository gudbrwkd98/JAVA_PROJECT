...
import com.shoppingmall.admin.board.Board;
...
public class Product extends Page{
	...
	public void getProduct(String name) {
		...
		if(name == null) {//name 값이 안넘어올시 모든 상품 가져오기
			...
			sql = "select p.product_id,p.subcategory_id,p.product_name,p.brand,p.price,p.filename,p.detail from product p inner join subcategory s on p.subcategory_id = s.subcategory_id inner join topcategory t on s.topcategory_id = t.topcategory_id where t.name = '"+name+"' OR s.name = '"+name+"' order by p.product_id desc";
		}
